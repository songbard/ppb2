...
UPDATE DURING NEW SCRIPT_1 TESTING: THE N=2 CASE DISAPPEARED.  LET'S SEE WHAT INCIDENT IT WAS...

```{r}
data_test29 <- ppb_force122[ppb_force122$recordcount_1_raw == 2, ]
data_test29
```
```{r}
remove(data_test29)
```
There are supposedly at least two FDRCs but where is #2?  Let's search on the incident ID.

```{r}
data_test29 <- ppb_force122[ppb_force122$incid_id_raw == 706548204, ] %>%
  print(n=20)
#data_test29
```
So this is the same incident for which 1 FDRC had recordcount2raw = 10 see below (BUT HAS RECORDCOUNT1raw = 2)  IN THIS CASE, RECOUNT_1 INDICATES THE RECORD WITH THE 2 IS THE SECOND FDRC SUBMITTED BY THE SAME OFFICER.  
